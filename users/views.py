    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    "No such phone number", status=status.HTTP_404_NOT_FOUND
                )
            ...
            if current_time < phone_otp.active_until and phone_otp.otp == otp:
                ...
                phone_otp.delete()
                return Response(
                    f"phone number {phone_otp.phone_number} is verified",
...
def add_to_queue(request, org_id, user_id):
    try:
        ...
        return Response(
            f"Something wrong with request", status=status.HTTP_400_BAD_REQUEST
        )
    ...
    return Response(
        f"{user} is added to queue with position {participant.position}",
        status=status.HTTP_200_OK,
    )
...
def process_current_pos(request, org_id):
    ...
    if queue.participants.count():
        ...
        participant.delete()
        return Response(
            f"{participant} was processed and current queue pos is  {queue.current_pos}",
            status=status.HTTP_200_OK,
        )
        ...
        if queue:
            ...
            data = {
                "waiting_time": calculate_time(
                    now, queue.num_servers, queue.max_service, queue.min_service
                )[1]
            }
            return Response(data, status=status.HTTP_200_OK)
...
class RemoveFromQueue(APIView):
    def post(self, request, user_id, org_id, **kwargs):
        ...
        if (queue and participant) and (
            queue.participants.filter(id=participant.id).exists()
        ):
            queue.participants.remove(participant)
            participant.delete()
            return Response("Participant was removed", status=status.HTTP_200_OK)
        return Response(
            "There is no such queue or participant", status=status.HTTP_200_OK
        )
...
class CreateUser(APIView):
    def post(self, request, **kwargs):
        ...
        try:
            ...
            return Response(
                "Something wrong with phonenumber", status=status.HTTP_400_BAD_REQUEST
            )
